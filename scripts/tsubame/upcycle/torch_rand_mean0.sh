...
#$ -l node_f=8
#$ -l h_rt=120:00:00
#$ -o outputs/upcycle/mixtral-8×1.56b_torch_rand_mean0/$JOB_ID
#$ -e outputs/upcycle/mixtral-8×1.56b_torch_rand_mean0/$JOB_ID
#$ -p -5
...
TOKENIZER_MODEL=/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x1.56B-torch_rand_mean0/tokenizer.model
CHECKPOINT_DIR=/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x1.56B-torch_rand_mean0
CHECKPOINT_SAVE_DIR="/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x1.56B-torch_rand_mean0_zero3/lr_${LR}-minlr_${MIN_LR}_warmup_${LR_WARMUP_STEPS}_seq_${SEQ_LENGTH}"
...
JOB_NAME="upcycle-8×1.56B-torch_rand_mean0-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
