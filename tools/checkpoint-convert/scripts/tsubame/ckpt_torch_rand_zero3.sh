...
BASE_MODEL_CHECKPOINT=/gs/bs/tgh-NII-LLM/checkpoints/upcycle-Mixtral-8x1.56B-torch_rand/

for (( ITERATION=$START_ITERATION; ITERATION<=$END_ITERATION; ITERATION+=6000 )); do
    FORMATTED_ITERATION=$(printf "iter_%07d" $ITERATION)

    CHECK_POINT_PATH=/gs/bs/tgh-NII-LLM/zero3_to_fp32/upcycle-Mixtral-8x1.56B-torch_rand_zero3/lr_2e-4-minlr_2e-5_warmup_2000_seq_4096/${FORMATTED_ITERATION}/model.pt
    OUTPUT_PATH=/gs/bs/tgh-NII-LLM/fp32_to_hf/upcycle-Mixtral-8x1.56B-torch_rand_zero3/lr_2e-4-minlr_2e-5_warmup_2000_seq_4096/${FORMATTED_ITERATION}
    mkdir -p $OUTPUT_PATH
